...
async function getNextSequenceValue(sequenceName: string, db: any): Promise<number> {
  const sequenceDocument = await db.collection('counters').findOneAndUpdate(
    { _id: sequenceName },
    { $inc: { sequence_value: 1 } },
    { returnDocument: 'after', upsert: true }
  );
  return sequenceDocument.sequence_value || sequenceDocument.value?.sequence_value || 1;
}
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'bulkAdd') {
      ...
      const result = await db.collection(table!).deleteMany({ id: { $in: ids.map(Number) } });
      return NextResponse.json(result);
