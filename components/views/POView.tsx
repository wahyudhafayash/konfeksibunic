...
export default function POView({
  ...
  const pos =
    useLiveQuery(() => db.pos.orderBy("id").reverse().toArray(), []) || [];
  ...
  async function handleSavePO(e: React.FormEvent) {
    ...
    if (poId) {
      ...
      const relatedItems = poItems
        .filter((i) => i.poId === poId)
        .map((i) => i.id!);
  ...
  const filteredPOs = pos.filter((p) => {
    ...
    const isVisibleInPO =
      computedStatus !== "Selesai" && computedStatus !== "Dihapus";

    return (
      matchesSearch &&
      matchesDate &&
      (filterStatus ? matchesStatus : isVisibleInPO)
    );
  });
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1 uppercase tracking-wider text-[10px]">
            Manajemen produksi barang & stok
          </p>
        </div>
          ...
          {filteredPOs.map((po) => {
            ...
            return (
              ...
              >
                {/* Thumbnail */}
                <div
                  className={`${viewMode === "list" ? "w-full md:w-48" : "w-full"} aspect-[4/3] md:aspect-auto overflow-hidden bg-slate-50 relative shrink-0`}
                >
                  {po.photoData ? (
                    <img
                      src={po.photoData}
                  ...
                  <div className="absolute top-4 left-4">
                    <span
                      className={`text-[10px] uppercase font-black tracking-widest px-3 py-1 rounded-full border shadow-sm backdrop-blur-md ${getPoStatus(po) === "Siap Kirim" ? "bg-emerald-500/90 text-white border-emerald-400" : "bg-slate-900/80 text-white border-white/20"}`}
                    >
                      {getPoStatus(po)}
                    </span>
                  </div>
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setStatusUpdatePOId(po.id!);
                          }}
                          className="p-2 text-slate-500 hover:bg-slate-100 rounded-xl transition-colors"
                          title="Perbarui Status Manual"
                        >
                          <Package size={16} />
                        </button>
                        ...
                        <button
                          onClick={(e) => openEditForm(e, po)}
                          className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors"
                        >
                          <Edit2 size={16} />
                        </button>
                        <button
                          onClick={(e) => requestDeletePO(e, po.id!)}
                      ...
                      <div className="hidden md:block">
                        <div className="flex flex-wrap gap-2">
                          {items.slice(0, 4).map((item, idx) => (
                            <span
                              key={idx}
                              className="text-[9px] font-bold bg-indigo-50 text-indigo-600 px-2 py-1 rounded-md border border-indigo-100"
                            >
      ...
      {filteredPOs.length === 0 && !isPOFormOpen && (
        ...
        >
          ...
          <p className="text-slate-500 font-medium mt-2">
            Daftar PO yang sedang dalam proses produksi akan tampil di sini.
          </p>
          <button
            onClick={openCreateForm}
            className="mt-8 bg-indigo-600 text-white font-bold px-8 py-3 rounded-2xl hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-600/20"
          >
            Tambah PO Sekarang
          </button>
        </motion.div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-end p-0 sm:p-4 bg-slate-950/60 backdrop-blur-sm no-print">
            ...
            >
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 tracking-tight">
                    {editId ? "Sempurnakan PO" : "Daftarkan PO Baru"}
                  </h3>
                  <p className="text-sm font-semibold text-slate-500 mt-1 uppercase tracking-widest text-[10px]">
                    Isi detail pesanan untuk memulai produksi
                  </p>
                </div>
                <button
                  onClick={() => setIsPOFormOpen(false)}
                  className="text-slate-400 hover:text-slate-900 bg-white p-3 rounded-2xl shadow-sm border border-slate-100 transition-all active:scale-95"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPoForm({ ...poForm, poNumber: e.target.value })
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPoForm({ ...poForm, customerName: e.target.value })
                      }
                      className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-3 text-sm font-bold focus:bg-white focus:border-indigo-500 focus:outline-none transition-all"
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setPoForm({ ...poForm, description: e.target.value })
                    }
                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-3 text-sm font-bold focus:bg-white focus:border-indigo-500 focus:outline-none transition-all"
                  ...
                  <div className="flex items-center gap-6 bg-slate-50 p-6 rounded-3xl border-2 border-slate-100 border-dashed group hover:border-indigo-200 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-bold mt-2 leading-relaxed">
                        Pilih foto referensi model untuk memudahkan tim produksi
                        memantau detail barang.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg">
                      Rincian Barang
                    </h4>
                    <button
                      type="button"
              ...
              <div className="p-8 border-t border-slate-100 bg-slate-50 flex justify-between items-center">
                ...
                >
                  <Save size={20} />{" "}
                  {editId ? "Simpan Update" : "Publish Order"}
                </button>
              </div>
            </motion.div>
          </div>
        ...
        {viewPOId && viewingPOInfo && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-md no-print">
            ...
            >
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-2 font-bold text-xs uppercase tracking-widest text-slate-400">
                      ...
                      <span>
                        {new Date(viewingPOInfo.date).toLocaleDateString(
                          "id-ID",
              ...
              <div
                className="overflow-y-auto p-10 bg-white grid grid-cols-1 lg:grid-cols-12 gap-10 custom-scrollbar"
                ref={printRef}
              >
                    ...
                    <div className="rounded-[2.5rem] overflow-hidden border-4 border-slate-100 shadow-xl bg-slate-50 aspect-[4/5]">
                      {viewingPOInfo.photoData ? (
                        <img
                          ...
                          onClick={() =>
                            setFullScreenPhoto(viewingPOInfo.photoData!)
                          }
                        />
                      ) : (
                  ...
                  <div className="bg-slate-50 rounded-[2rem] p-8 border border-slate-100 space-y-6">
                    <div>
                      ...
                      <p className="text-xl font-extrabold text-slate-900 tracking-tight">
                        {viewingPOInfo.customerName || "Customer Umum"}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
                      ...
                      <div className="mt-2 flex items-center gap-3">
                        <span
                          className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${getPoStatus(viewingPOInfo) === "Siap Kirim" ? "bg-emerald-500 text-white border-emerald-400" : "bg-slate-900 text-white border-slate-700"}`}
                        >
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-600 leading-relaxed italic">
                        {viewingPOInfo.description ||
                          "Tidak ada instruksi khusus untuk pesanan ini."}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-6">
                      <h4 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                        <List size={22} className="text-indigo-600" /> Rincian
                        Barang & Material
                      </h4>
                      ...
                      <table className="w-full text-left border-separate border-spacing-y-2">
                        <thead>
                          <tr className="bg-slate-100 rounded-xl text-[10px] font-black text-slate-500 uppercase tracking-widest">
                            ...
                            <th className="px-6 py-4 text-right rounded-r-2xl">
                              Kuantitas
                            </th>
                          </tr>
                        </thead>
                        <tbody className="text-sm font-bold">
                          {viewingPOItems.map((item, idx) => (
                            <tr
                              key={item.id}
                              className="bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow group"
                            >
                              <td className="px-6 py-5 rounded-l-2xl border-y border-l border-slate-100">
                                <div className="flex items-center gap-4">
                                  ...
                                  <span className="text-slate-900 group-hover:text-indigo-600 transition-colors uppercase">
                                    {item.itemName}
                                  </span>
                                </div>
                              </td>
                              <td className="px-6 py-5 border-y border-slate-100">
                                <div className="flex gap-2">
                                  <span className="bg-slate-100 px-3 py-1 rounded-lg text-[10px] text-slate-600 uppercase tabular-nums">
                                    {item.color}
                                  </span>
                                  <span className="bg-slate-100 px-3 py-1 rounded-lg text-[10px] text-slate-600 uppercase tabular-nums">
                                    SIZE {item.size}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-3xl font-black tracking-tighter leading-none">
                            {viewingPOItems.reduce((acc, i) => acc + i.qty, 0)}{" "}
          ...
          <div className="fixed inset-0 z-[80] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md">
            ...
            >
              ...
              <div className="space-y-3">
                {["Proses", "Siap Kirim", "Selesai", "Dihapus"].map(
                  (status) => (
                    <button
                      key={status}
                      onClick={() => updatePOStatus(statusUpdatePOId, status)}
                      className={`w-full py-4 px-6 rounded-2xl border-2 transition-all font-bold text-left flex justify-between items-center group ${
                        pos.find((p) => p.id === statusUpdatePOId)?.status ===
                        status
              ...
              <button
                onClick={() => setStatusUpdatePOId(null)}
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md">
            ...
            >
              ...
              <p className="text-sm font-medium text-slate-500 mb-8">
                Yakin ingin menyelesaikan PO ini? PO akan dipindahkan ke
                histori/arsip sebagai catatan final.
              </p>
              <div className="flex gap-3">
                <button
        ...
        {deletePOId && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md">
            ...
            >
              ...
              <p className="text-sm font-medium text-slate-500 mb-8">
                Apakah Anda yakin ingin menghapus PO ini? PO akan terhapus dan
                dipindahkan ke histori/arsip.
              </p>
              <div className="flex gap-3">
                <button
