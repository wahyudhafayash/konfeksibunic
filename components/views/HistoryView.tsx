...
import { db, useLiveQuery, PO, SalaryPayment, TabunganWithdrawal } from '@/lib/db';
import { Search, ChevronRight, CheckCircle2, Wallet, PiggyBank, Package, Clock, Filter, Calendar, X, Eye, EyeOff, Printer, Info, Download, ArrowRight, User, TrendingUp } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export default function HistoryView() {
  const [activeTab, setActiveTab] = useState<'po' | 'salary' | 'tabungan' | 'tailors'>('po');
  ...
  const [deletedTailorFilter, setDeletedTailorFilter] = useState<{fromDate: string, toDate: string, sortBy: 'terbaru' | 'terlama'}>({ fromDate: '', toDate: '', sortBy: 'terbaru' });

  const printRef = useRef<HTMLDivElement>(null);

  const pos = useLiveQuery(() => db.pos.toArray(), []) || [];
  const poItems = useLiveQuery(() => db.poItems.toArray(), []) || [];
  const salaryPayments = useLiveQuery(() => db.salaryPayments.orderBy('date').reverse().toArray(), []) || [];
  const tailors = useLiveQuery(() => db.tailors.toArray(), []) || [];
  const withdrawals = useLiveQuery(() => db.tabunganWithdrawals.orderBy('date').reverse().toArray(), []) || [];
  ...
  const completedPos = pos.filter(p => {
    ...
    return (p.status === 'Selesai' || p.status === 'Dihapus') && matchesSearch && matchesDate;
  });
  
  const filteredSalaries = salaryPayments.filter(s => {
    const matchesSearch = getTailorName(s.tailorId).toLowerCase().includes(searchQuery.toLowerCase());
    const matchesDate = isWithinDateRange(s.date);
    const matchesTailor = selectedTailorId ? s.tailorId === selectedTailorId : true;
    return matchesSearch && matchesDate && matchesTailor;
  });
  
  const filteredWithdrawals = withdrawals.filter(w => {
    const matchesSearch = getTailorName(w.tailorId).toLowerCase().includes(searchQuery.toLowerCase());
    const matchesDate = isWithinDateRange(w.date);
    const matchesTailor = selectedTailorId ? w.tailorId === selectedTailorId : true;
    return matchesSearch && matchesDate && matchesTailor;
  });

  const deletedTailors = tailors.filter(t => t.status === 'Dihapus' && (
  ...
  return (
    <div className="space-y-10 pb-24">
       <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">Histori & Arsip</h2>
            <p className="text-slate-500 font-medium mt-1 uppercase tracking-widest text-[10px]">Basis data riwayat transaksi dan log produksi</p>
          </div>
       ...
       <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-8 shadow-sm flex flex-col gap-6">
          ...
          <div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl self-start w-full sm:w-auto overflow-x-auto custom-scrollbar flex-wrap">
             <button onClick={() => setActiveTab('po')} className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'po' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Arsip PO</button>
             <button onClick={() => setActiveTab('salary')} className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'salary' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Penggajian</button>
             <button onClick={() => setActiveTab('tabungan')} className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'tabungan' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Tabungan</button>
             <button onClick={() => setActiveTab('tailors')} className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'tailors' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Penjahit Dihapus</button>
          </div>
       </div>
       ...
       <motion.div variants={containerVariants} initial="hidden" animate="visible" key={activeTab}>
          {activeTab === 'po' && (
             <div className="space-y-12">
                {completedPos.length === 0 ? (
                   <div className="text-center py-20 opacity-40 italic font-bold">Data PO tidak ditemukan.</div>
                ) : (
                   Object.entries(
                     completedPos.reduce((acc, po) => {
                       const name = po.customerName;
                       if (!acc[name]) acc[name] = [];
                       acc[name].push(po);
                       return acc;
                     }, {} as Record<string, PO[]>)
                   ).map(([customerName, posInGroup]) => (
                     <div key={customerName} className="space-y-6">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {posInGroup.map(po => {
                             ...
                             return (
                                <motion.div variants={itemVariants} key={po.id} className="bg-white border-2 border-slate-50 rounded-[2.5rem] p-6 shadow-sm hover:border-emerald-100 hover:shadow-md transition-all group relative overflow-hidden">
                                   <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                      <button onClick={() => setSelectedPO(po)} className="bg-white border border-slate-200 p-2 rounded-xl text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all shadow-sm">
                                         <Info size={16}/>
                                      </button>
                                   </div>
                                   <div className="mb-6">
                                      <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-1 block">{formatDate(po.date)}</span>
          ...
          {activeTab === 'salary' && (
             <div className="space-y-12">
                {filteredSalaries.length === 0 ? (
                   <div className="text-center py-20 opacity-40 italic font-bold">Data gaji tidak ditemukan.</div>
                ) : (
                   Object.entries(
                     filteredSalaries.reduce((acc, sal) => {
                       const name = getTailorName(sal.tailorId);
                       if (!acc[name]) acc[name] = [];
                       acc[name].push(sal);
                       return acc;
                     ...
                     <div key={name} className="space-y-6">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                         {payments.map(sal => (
                           <motion.div variants={itemVariants} key={sal.id} onClick={() => setSelectedSalary(sal)} className="bg-white border-2 border-slate-50 rounded-[2.5rem] p-8 shadow-sm hover:border-indigo-100 hover:shadow-md transition-all cursor-pointer group">
                              <div className="flex justify-between items-start mb-6">
                                 <div>
                                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-1">{formatDate(sal.date)}</p>
                                    <h4 className="font-black text-slate-900 text-lg uppercase tracking-tight">Slip #{sal.id}</h4>
                                 </div>
                              ...
                              <div className="space-y-4 mb-8">
                                 ...
                                 <div className="flex justify-between items-center text-xs">
                                    ...
                                    <span className="font-black text-slate-700">{displayAmount(sal.totalWage, `sal_wage_${sal.id}`)}</span>
                                 </div>
                                 <div className="flex justify-between items-center text-xs text-rose-500">
                                    <span className="font-bold">Pot. Kasbon</span>
                                    <span className="font-black">-{displayAmount(sal.kasbonDeducted, `sal_kasbon_${sal.id}`)}</span>
                                 </div>
                              </div>

                              <div className="pt-6 border-t-2 border-slate-50 flex justify-between items-end">
                                 <div>
                                    <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">Diterima</p>
                                    <p className="text-2xl font-black text-indigo-600 tabular-nums leading-none">{displayAmount(sal.netPayment, `sal_net_${sal.id}`)}</p>
                                 </div>
                                 <ArrowRight size={20} className="text-slate-300 group-hover:text-indigo-600 transition-colors group-hover:translate-x-1 transition-transform" />
                              </div>
                           </motion.div>
                         ))}
                       </div>
          ...
          {activeTab === 'tabungan' && (
             <div className="space-y-12">
                {filteredWithdrawals.length === 0 ? (
                   <div className="text-center py-20 opacity-40 italic font-bold">Data pencairan tabungan tidak ditemukan.</div>
                ) : (
                   Object.entries(
                     filteredWithdrawals.reduce((acc, wd) => {
                       const name = getTailorName(wd.tailorId);
                       if (!acc[name]) acc[name] = [];
                       acc[name].push(wd);
                       return acc;
                     }, {} as Record<string, TabunganWithdrawal[]>)
                   ).map(([name, withdrawalsInGroup]) => (
                     <div key={name} className="space-y-6">
                       <h3 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-3 px-2">
                         <div className="w-1.5 h-6 bg-sky-500 rounded-full"></div> {name}
                       </h3>
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {withdrawalsInGroup.map(wd => (
                             <motion.div variants={itemVariants} key={wd.id} className="bg-white border-2 border-slate-50 rounded-[2.5rem] p-6 shadow-sm hover:border-sky-100 transition-all flex items-center justify-between group">
                                <div className="flex items-center gap-5">
                                   ...
                                   <div>
                                      <p className="text-[10px] font-black text-sky-400 uppercase tracking-widest mb-1">{formatDate(wd.date)}</p>
                                ...
                                <div className="text-right">
                                   ...
                                   <p className="text-xl font-black text-sky-600 tabular-nums leading-none">{displayAmount(wd.amount, `wd_${wd.id}`)}</p>
                                </div>
                             </motion.div>
                          ))}
                       </div>
          ...
          {activeTab === 'tailors' && (
             <div className="space-y-6">
                {deletedTailors.length === 0 ? (
                   <div className="text-center py-20 opacity-40 italic font-bold">Data penjahit dihapus tidak ditemukan.</div>
                ) : (
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {deletedTailors.map(t => {
                         const tailorSalaries = salaryPayments.filter(s => s.tailorId === t.id);
                         const tailorJobs = jobs.filter(j => j.tailorId === t.id);
                         const totalJobs = tailorJobs.length;
                         const totalGajiDiterima = tailorSalaries.reduce((sum, s) => sum + s.netPayment, 0);

                         return (
                            <motion.div variants={itemVariants} key={t.id} className="bg-white border-2 border-slate-50 rounded-[2.5rem] p-6 shadow-sm hover:shadow-md transition-all">
                               <div className="flex items-center gap-4 mb-6">
                                  ...
                                  <div>
                                     ...
                                     <div className="flex items-center gap-2 mt-2">
                                        <span className="bg-rose-100 text-rose-700 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">Dihapus</span>
                                     </div>
                                  </div>
                               </div>
                               <div className="space-y-4 pt-4 border-t border-slate-50 border-dashed">
                                  <div className="flex justify-between items-center bg-slate-50/50 px-4 py-2.5 rounded-xl">
                                     <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Tugas Dikerjakan</span>
                                     <span className="text-sm font-black text-slate-700">{totalJobs} TASK</span>
                                  </div>
                                  <div className="flex justify-between items-center bg-slate-50/50 px-4 py-2.5 rounded-xl">
                                     <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Penghasilan</span>
                                     <span className="text-sm font-black text-indigo-600">{displayAmount(totalGajiDiterima, `dt_${t.id}`)}</span>
                                  </div>
                               </div>
                               <div className="mt-4">
                                  <button onClick={() => setSelectedDeletedTailor(t)} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all">
                                    Lihat Detail
                                  </button>
                               </div>
                            </motion.div>
             ...
             >
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                         <h3 className="font-black text-slate-900 text-2xl tracking-tight leading-none uppercase">#{selectedPO.poNumber}</h3>
                         <div className="flex items-center gap-2 mt-2">
                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest bg-white border border-slate-100 px-3 py-1 rounded-lg inline-block">Client: {selectedPO.customerName}</p>
                            {selectedPO.createdBy && (
                               <span className="text-[9px] text-slate-400 font-bold italic uppercase">By: {selectedPO.createdBy}</span>
                            )}
                         </div>
                      </div>
                   </div>
                   <div className="flex gap-3">
                      <button onClick={() => handleDownloadPOCSV(selectedPO)} className="bg-white border-2 border-slate-100 text-slate-600 hover:text-emerald-600 hover:border-emerald-100 px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-sm flex items-center gap-2">
                         <Download size={18}/> Export CSV
                      </button>
                      <button onClick={handlePrint} className="bg-slate-900 text-white hover:bg-emerald-600 px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-slate-900/10 transition-all flex items-center gap-2">
                         <Printer size={18}/> Cetak Nota
                      </button>
                      <button onClick={() => setSelectedPO(null)} className="text-slate-400 hover:text-slate-900 bg-white border border-slate-100 p-3 rounded-2xl transition-all">
                         <X size={24}/>
                      </button>
                   </div>
                </div>
                <div className="overflow-y-auto flex-1 p-10 custom-scrollbar" ref={printRef}>
                   <div className="space-y-10">
                      ...
                      <div className="hidden print:block text-center border-b pb-8 mb-8">
                         <h1 className="text-3xl font-black uppercase tracking-[0.2em] text-slate-900">KONFEKSI APP</h1>
                         <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2 px-10">Laporan Riwayat Arsip Pesanan (Final Record)</p>
                      </div>

                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                         <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nomor Order</p>
                            <p className="font-black text-slate-900 text-lg uppercase tracking-tight">{selectedPO.poNumber}</p>
                         </div>
                         <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nama Pelanggan</p>
                            <p className="font-black text-slate-900 text-lg uppercase tracking-tight">{selectedPO.customerName}</p>
                         </div>
                         <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Tanggal Berjalan</p>
                            <p className="font-bold text-slate-700 text-lg">{formatDate(selectedPO.date)}</p>
                         </div>
                         <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Status Akhir</p>
                      ...
                      <div className="mt-12">
                         <h4 className="text-sm font-black text-slate-400 uppercase tracking-[0.2em] mb-6 border-b border-slate-50 pb-4">Daftar Item & Rincian Produksi</h4>
                            ...
                            <table className="w-full text-left border-collapse text-sm">
                               <thead className="bg-slate-50 border-b border-slate-100 font-bold text-slate-500 uppercase tracking-widest text-[9px]">
                                  <tr>
                                     ...
                                     <th className="px-8 py-5 text-center">Volume (PCS)</th>
                                  </tr>
                               </thead>
                               <tbody className="divide-y divide-slate-50">
                                  {poItems.filter(it => it.poId === selectedPO.id).map(item => (
                                     <tr key={item.id} className="hover:bg-slate-50/30 transition-colors">
                                        <td className="px-8 py-5 font-black text-slate-900 uppercase tracking-tight">{item.itemName}</td>
                                        <td className="px-8 py-5 font-bold text-slate-600 uppercase">{item.color}</td>
                                        <td className="px-8 py-5 font-bold text-slate-600 uppercase">{item.size}</td>
                                        <td className="px-8 py-5 font-black text-slate-900 text-center text-lg">{item.qty}</td>
                                     </tr>
                                  ))}
                               </tbody>
                            </table>
                      ...
                      <div className="pt-12 mt-12 border-t-2 border-slate-50 hidden print:block">
                         <div className="flex justify-between">
                            <div className="text-center w-48">
                               <p className="text-[10px] font-black uppercase tracking-widest mb-16">Dibuat Oleh,</p>
                               <div className="w-full border-b-2 border-slate-100"></div>
                               <p className="text-[10px] font-bold text-slate-400 mt-2 uppercase tracking-widest">Administrator</p>
                            </div>
                            <div className="text-center w-48">
                               <p className="text-[10px] font-black uppercase tracking-widest mb-16">Penerima,</p>
                               <div className="w-full border-b-2 border-slate-100"></div>
                               <p className="text-[10px] font-bold text-slate-400 mt-2 uppercase tracking-widest">{selectedPO.customerName}</p>
                            </div>
                         </div>
                         <p className="text-center italic text-[9px] text-slate-300 mt-12">Laporan ini dihasilkan secara otomatis oleh sistem KonfeksiApp pada {new Date().toLocaleString('id-ID')}</p>
                      </div>
                   </div>
                </div>
             </motion.div>
             ...
             >
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                   <div className="flex items-center gap-5">
                      ...
                      <div>
                         <h3 className="font-black text-slate-900 text-2xl tracking-tight leading-none uppercase">SLIP GAJI: {getTailorName(selectedSalary.tailorId)}</h3>
                         <div className="flex items-center gap-2 mt-2">
                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest bg-white border border-slate-100 px-3 py-1 rounded-lg inline-block">Payment Ref: #{selectedSalary.id}</p>
                ...
                <div className="overflow-y-auto flex-1 p-10 custom-scrollbar" ref={printRef}>
                   <div className="space-y-8 bg-white max-w-xl mx-auto p-8 border-2 border-slate-50 rounded-[3rem] shadow-sm relative">
                      ...
                      <div className="text-center flex flex-col items-center mb-10 pt-4">
                         ...
                         <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">Official Remuneration Slip</p>
                      </div>

                      <div className="grid grid-cols-2 gap-8 border-y-2 border-slate-50 border-dashed py-8">
                         <div>
                            <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">Nama Pekerja</p>
                            <p className="font-black text-slate-900 tracking-tight text-lg uppercase">{getTailorName(selectedSalary.tailorId)}</p>
                         </div>
                         <div className="text-right">
                            <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">Tanggal Transaksi</p>
                            <p className="font-bold text-slate-700">{formatDate(selectedSalary.date)}</p>
                         </div>
                      </div>
                      ...
                      <div className="space-y-4">
                         <div className="flex justify-between items-center group">
                            <div className="flex flex-col">
                               <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Upah Borongan</span>
                               <span className="text-[9px] font-bold text-slate-300">{selectedSalary.totalQty} PCS Dikerjakan</span>
                            </div>
                            <span className="text-lg font-black text-slate-900 tabular-nums">
                               {displayAmount(selectedSalary.totalWage, `psw_${selectedSalary.id}`)}
                            </span>
                         </div>
                         
                         <div className="flex justify-between items-center text-emerald-600">
                            <div className="flex flex-col">
                               <span className="text-[10px] font-black uppercase tracking-widest">Bonus Grade Produksi</span>
                               <span className="text-[9px] font-bold opacity-60">Level: {selectedSalary.gradeName}</span>
                            </div>
                            <span className="text-lg font-black tabular-nums">
                               +{displayAmount(selectedSalary.bonusAmount, `psb_${selectedSalary.id}`)}
                            </span>
                         </div>

                         {selectedSalary.manualAdjustment !== 0 && (
                            <div className={`flex justify-between items-center ${selectedSalary.manualAdjustment > 0 ? 'text-purple-600' : 'text-rose-500'}`}>
                               <div className="flex flex-col">
                                  <span className="text-[10px] font-black uppercase tracking-widest">Penyesuaian Manual</span>
                                  <span className="text-[9px] font-bold opacity-60">{selectedSalary.manualNote || 'Adjustment'}</span>
                               </div>
                               <span className="text-lg font-black tabular-nums font-mono">
                                  {selectedSalary.manualAdjustment > 0 ? '+' : ''}{displayAmount(selectedSalary.manualAdjustment, `psa_${selectedSalary.id}`)}
                               </span>
                            </div>
                         )}
                         ...
                         <div className="flex justify-between items-center text-rose-500 py-4 border-t border-slate-50 border-dashed">
                            <div className="flex flex-col">
                               <span className="text-[10px] font-black uppercase tracking-widest">Pelunasan Kasbon</span>
                               <span className="text-[9px] font-bold opacity-60">Automatic Deduction</span>
                            </div>
                            <span className="text-lg font-black tabular-nums">
                               -{displayAmount(selectedSalary.kasbonDeducted, `psk_${selectedSalary.id}`)}
                            </span>
                         </div>

                         <div className="pt-6 border-t-2 border-slate-50 flex justify-between items-end">
                            <div>
                               <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-1">Diterima Bersih</p>
                               <p className="text-[10px] font-bold text-slate-300">Net Take Home Pay</p>
                            </div>
                            <p className="text-4xl font-black text-indigo-600 tabular-nums tracking-tighter shadow-sm">
                               {displayAmount(selectedSalary.netPayment, `psn_${selectedSalary.id}`)}
                            </p>
                         </div>
                      </div>
                      ...
                      {selectedSalary.tabunganAccumulated > 0 && (
                         <div className="bg-sky-50 border-2 border-sky-100 rounded-[2rem] p-5 flex justify-between items-center">
                            ...
                            <span className="font-black text-sky-700 text-lg tabular-nums">
                               +{displayAmount(selectedSalary.tabunganAccumulated, `pst_${selectedSalary.id}`)}
                            </span>
                         </div>
                      )}
                      ...
                      <div className="pt-12 mt-12 border-t-2 border-slate-50 hidden print:block">
                         <div className="flex justify-between">
                            <div className="text-center w-40">
                               <p className="text-[9px] font-black uppercase tracking-widest mb-16">Finance,</p>
                               <div className="w-full border-b border-slate-200"></div>
                            </div>
                            <div className="text-center w-40">
                               <p className="text-[9px] font-black uppercase tracking-widest mb-16">Penerima,</p>
                               <div className="w-full border-b border-slate-200"></div>
                            </div>
                         </div>
                      </div>
             ...
             >
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                   <div className="flex items-center gap-5">
                      ...
                      <div>
                         <h3 className="font-black text-slate-900 text-2xl tracking-tight leading-none uppercase">DETAIL HISTORI: {selectedDeletedTailor.name}</h3>
                         <div className="flex items-center gap-2 mt-2">
                            <span className="text-[10px] text-rose-500 font-black uppercase tracking-widest bg-rose-50 px-3 py-1 rounded-lg inline-block">PENJAHIT DIHAPUS</span>
                         </div>
                      </div>
                   </div>
                   <div className="flex items-center gap-2">
                      <button onClick={() => setSelectedDeletedTailor(null)} className="text-slate-400 hover:text-slate-900 bg-white p-3 rounded-2xl border border-slate-200 shadow-sm hover:shadow transition-all">
                         <X size={24}/>
                      </button>
                   </div>
                </div>
                ...
                <div className="p-8 overflow-y-auto custom-scrollbar flex-1 bg-slate-50/30">
                   {(() => {
                      let tailorJobs = jobs.filter(j => j.tailorId === selectedDeletedTailor.id);
                      
                      if (deletedTailorFilter.fromDate) {
                         const from = new Date(deletedTailorFilter.fromDate);
                         from.setHours(0, 0, 0, 0);
                         tailorJobs = tailorJobs.filter(j => new Date(j.dateTaken) >= from);
                      }
                      if (deletedTailorFilter.toDate) {
                         const to = new Date(deletedTailorFilter.toDate);
                         to.setHours(23, 59, 59, 999);
                         tailorJobs = tailorJobs.filter(j => new Date(j.dateTaken) <= to);
                      }
                      ...
                      if (tailorJobs.length === 0) {
                         return (
                            <div className="space-y-4">
                               ...
                               <div className="text-center py-20 text-slate-400 font-bold italic">Belum ada tugas yang dikerjakan pada rentang ini.</div>
                            </div>
                         );
                      }
                      ...
                      return (
                         <div className="space-y-4">
                            ...
                            <h4 className="text-sm font-black text-slate-500 uppercase tracking-widest pl-2 border-l-4 border-rose-500">History Pekerjaan</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                               {tailorJobs.map(j => {
                                  const progress = Math.round((j.qtySubmitted / j.qtyTaken) * 100);
                                     ...
                                     <div key={j.id} className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm hover:shadow transition-all relative">
                                        ...
                                        <div className="grid grid-cols-2 gap-3 mb-4">
                                           ...
                                           <div className="bg-emerald-50/50 p-3 rounded-2xl border border-emerald-50/50">
                                              ...
                                              <span className="font-black text-emerald-700">{j.qtySubmitted} / {j.qtyTaken}</span>
                                           </div>
                                        </div>
